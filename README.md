...
**<фамилия>.<имя>**, 
набранным строчными латинскими символами (например ```ivanov.ivan```).
...
**<фамилия>.<имя>/<работа> (ivanov.ivan/T0)**.
+ Для того, чтобы работа была принята, необходимо создать **pull request**.
На каждую работу должен быть создан свой **pull request** и при том только один!
...
+ Для отступов должны использоваться только пробелы, не табуляции!
(Для **VisualStudio Tools -> Options -> Text Editor -> [язык] -> Tabs -> Insert spaces**)
+ Окончания строк и конца файла должны быть в unix-style, последним символом в каждом файле должен быть символ 
перевода строки. 
Те, кто использует Git в Windows, могут установить указанную ниже настройку или выбрать 
соответствующий пункт при установке ([руководство](https://git-scm.com/book/ru/v2/%D0%9D%D0%B0%D1%81%D1%82%D1%80%D0%BE%D0%B9%D0%BA%D0%B0-Git-%D0%9A%D0%BE%D0%BD%D1%84%D0%B8%D0%B3%D1%83%D1%80%D0%B0%D1%86%D0%B8%D1%8F-Git))
...
+ Скачайте и установите Git на свой компьютер с [сайта](https://git-scm.com/install/) или через [менеджер пакетов](https://git-scm.com/book/ru/v2/%D0%92%D0%B2%D0%B5%D0%B4%D0%B5%D0%BD%D0%B8%D0%B5-%D0%A3%D1%81%D1%82%D0%B0%D0%BD%D0%BE%D0%B2%D0%BA%D0%B0-Git).
...
+ Вы можете установить [графический интерфейс](https://git-scm.com/tools/guis), например SourceTree, или воспользоваться средствами, встроенными в IDE.
...
+ Добавьте основной репозиторий в список удалённых (remote):
  ...
  ```git remote add upstream https://github.com/ov-ale/tp2026.git```
...
+ Создать папку для новой работы. Сделать это можно разными способами, в т.ч. и через IDE, но отправлять 
на проверку файлы, созданные IDE **не нужно**.
+ Создать main.cpp, содержащий компилирующуюся функцию main() и добавить его в Git:
...
После отправки готовой работы в свой удалённый репозиторий (fork), необходимо отправить её на проверку в 
основной репозиторий. Для этого средствами GitHub создайте pull request из соответствующей ветки fork'а 
в master ветку основного репозитория.

Также для проверки выполнения тестов можно создать pull request в master ветку fork'а, **но принимать 
(merge) этот pull request не следует!** Для активации тестов необходимо нажать кнопку "Enable..." на странице Actions.

## Синхронизация с основным репозиторием

Работы должны сдаваться по порядку. После того, как ваш pull request принят (merge) в основном репозитории, 
необходимо синхронизировать master-ветки fork и основного репозитория:
...
+ На проекте используется Continuous Integration (CI), соответственно, сборка и тестирование выполняются автоматически. 
Коммиты, не прошедшие CI (с ошибкой или пропустившие CI) не принимаются.
...
Makefile автоматически строит программу для каждой работы. Так как имя программы в общем случае неизвестно, 
специальная цель выполняет запуск построенной программы с передачей параметров.
...
Дополнительной возможностью является запуск динамического анализатора [Valgrind](https://valgrind.org) для запускаемых программ. 
